...
router.get('/:subject_id/:grade_id', async (req, res) => {
  ...
  try {
    const getLessons = 'SELECT id, name FROM LESSON WHERE subject_id = ? AND grade_id = ?';
...
router.delete('/:id', (req, res) => {
  ...
  db.query(deleteDocs, [id], (err, results) => {
    if (err) {
      ...
      res.status(500).json({ error: 'An error occurred while deleting the documents.' });
    } else {
      ...
      db.query(deleteLessonSQL, [id], (err, results) => {
        if (err) {
          ...
          res.json({ message: 'Lesson and associated documents deleted successfully.' });
        }
      });
    }
  });
...
router.post('/', async (req, res) => {
  ...
  const addLesson = 'INSERT INTO lesson (name, subject_id, grade_id) VALUES (?, ?, ?)';
