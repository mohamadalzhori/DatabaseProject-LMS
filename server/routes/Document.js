...
router.get('/:lesson_id', async (req, res) => {
  ...
  try {
    const getDocs = 'SELECT id, title, description, URL FROM DOCUMENT WHERE lesson_id = ?';
...
router.post('/', async (req, res) => {
    ...
    const addDoc = 'INSERT INTO document (title, description, URL, lesson_id) VALUES (?, ?, ?, ?)';
...
router.patch('/:id', async (req, res) => {
    ...
    try {
      const updateDocumentSQL = 'UPDATE DOCUMENT SET title = ?, description = ?, URL = ? WHERE id = ?';
  
      db.query(updateDocumentSQL, [updatedDocumentData.title, updatedDocumentData.description, updatedDocumentData.URL, documentId], (err, results) => {
        if (err) {
